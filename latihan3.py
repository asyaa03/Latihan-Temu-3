def jumlah_hari(bulan):
    ...
    # Nama bulan ke angka
    nama_ke_bulan = {v[0].lower(): x for x, v in hari_per_bulan.items()}

    # Jika input adalah angka
    if isinstance(bulan, int):
        if 1 <= bulan <= 12:
            nama_bulan, jumlah_hari = hari_per_bulan[bulan]
            return f"Bulan yang Kamu masukkan adalah {nama_bulan}, yang memiliki {jumlah_hari} hari."
        else:
            return "Bulan yang Kamu masukkan tidak valid. Masukkan angka antara 1 dan 12."
    ...
    # Jika input adalah string
        ...
        if bulan in nama_ke_bulan:
            ...
            return f"Bulan ke-{bulan_angka} adalah {nama_bulan} memiliki {jumlah_hari} hari."
        else:
            return "Nama bulan yang Kamu masukkan tidak valid. Masukkan nama bulan dengan benar."
    else:
